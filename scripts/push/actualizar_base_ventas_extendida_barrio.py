# actualizar_base_ventas_extendida_barrio.py
import os
import sys
import logging
...
# Prefect flow
# =========================
@flow(name="actualizar_base_ventas_extendida_dbarrio")
def actualizar_base_ventas_extendida_barrio(window_days: int = 14, analyze: bool = True):
    """
    ...
    """
    log = get_run_logger()
    log.info(f"[INICIO] actualizar_base_ventas_extendida_dbarrio | ventana={window_days} días")
    ...
    log.info(f"[FIN] actualizar_base_ventas_extendida_dbarrio | DQ={dq}")
...
if __name__ == "__main__":
    # Uso:
    #   python actualizar_base_ventas_extendida_barrio.py            -> ventana default 14
    #   python actualizar_base_ventas_extendida_barrio.py 7 false    -> 7 días, sin ANALYZE
    wdays = int(sys.argv[1]) if len(sys.argv) >= 2 else 14
    do_analyze = True
    if len(sys.argv) >= 3:
        arg = sys.argv[2].strip().lower()
        do_analyze = (arg in ("true", "1", "yes", "y", "t"))
    actualizar_base_ventas_extendida_barrio(window_days=wdays, analyze=do_analyze)
    logger.info("--------------->  Flujo actualizar_base_ventas_extendida_dbarrio FINALIZADO.")
