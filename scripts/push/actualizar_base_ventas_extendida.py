...
DEFAULT_REPROCESS_MODE = "upsert_rango"
REPROCESS_MODES = {DEFAULT_REPROCESS_MODE, "replace_rango"}
...
def parse_input_date(value, field_name: str) -> date:
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(f"{field_name} debe estar en formato YYYY-MM-DD. Valor recibido: {value}") from exc
    raise TypeError(f"{field_name} debe ser str, date o datetime. Valor recibido: {type(value).__name__}")
...
def resolve_runtime_mode(
    ...
    fecha_desde=None,
    fecha_hasta=None,
    modo_reproceso: Optional[str] = None,
):
    if (fecha_desde is None) != (fecha_hasta is None):
        raise ValueError("Para reproceso histórico se deben informar ambas fechas: fecha_desde y fecha_hasta.")

    if fecha_desde is None and fecha_hasta is None:
        if modo_reproceso:
            raise ValueError("modo_reproceso solo aplica cuando se informan fecha_desde y fecha_hasta.")
        if window_days < 0:
        ...
        return {
            ...
            "fecha_desde": None,
            "fecha_hasta": None,
            "fecha_hasta_exclusive": None,
            "modo_reproceso": None,
            "description": f"ventana={window_days} días",
        }

    start_date = parse_input_date(fecha_desde, "fecha_desde")
    end_date = parse_input_date(fecha_hasta, "fecha_hasta")
    if start_date > end_date:
        raise ValueError("fecha_desde no puede ser mayor que fecha_hasta.")

    mode = (modo_reproceso or DEFAULT_REPROCESS_MODE).strip().lower()
    if mode not in REPROCESS_MODES:
        modos = ", ".join(sorted(REPROCESS_MODES))
        raise ValueError(f"modo_reproceso inválido: {modo}. Valores permitidos: {modos}.")
    ...
    return {
        ...
        "fecha_hasta_exclusive": end_date + timedelta(days=1),
        "modo_reproceso": mode,
        "description": f"rango={start_date}..{end_date} | modo={mode}",
    }
...
LEFT JOIN src.t050_articulos A
       ON V.c_articulo = A.c_articulo
WHERE V.f_venta::date >= %s
  AND V.f_venta::date < %s
  AND V.c_sucu_empr < 300
...
SQL_DELETE_DEST_RANGE = """
DELETE FROM src.base_ventas_extendida
WHERE fecha >= %s AND fecha < %s;
"""
...
def t_insert_stg(window_days: int = 14, fecha_desde=None, fecha_hasta=None):
    def _run():
        runtime = resolve_runtime_mode(window_days=window_days, fecha_desde=fecha_desde, fecha_hasta=fecha_hasta)
        with open_pg_conn() as conn:
            configure_session(conn)
            with conn.cursor() as cur:
                if runtime["strategy"] == "window":
                    exec_sql(
                        cur,
                        SQL_INSERT_STG_WINDOW,
                        params=(window_days,),
                        log_prefix=f"Insert STG ({runtime['description']})",
                    )
                else:
                    exec_sql(
                        cur,
    ...
    execute_with_retry(_run, "Insertar STG")
...
@task(name="Replace_Destino_Rango")
def t_replace_destino_rango(fecha_desde, fecha_hasta):
    start_date = parse_input_date(fecha_desde, "fecha_desde")
    end_date = parse_input_date(fecha_hasta, "fecha_hasta")
        ...
        with open_pg_conn() as conn:
            ...
            with conn.cursor() as cur:
                exec_sql(
                    ...
                    log_prefix=f"Delete DESTINO rango {start_date}..{end_date}",
                )
            conn.commit()
        logger.info("Destino depurado para replace_rango.")

    execute_with_retry(_run, f"Replace destino rango [{start_date}..{end_date}]")
...
def actualizar_base_ventas_extendida(
    window_days: int = 14,
    analyze: bool = True,
    ...
    Mejoras incorporadas:
      ...
      6) Cobertura unificada para ventas estándar y Barrio.
      7) Reproceso histórico opcional por rango exacto.

    Modos de ejecución:
      - Diario: usar window_days.
      - Histórico puntual: usar fecha_desde y fecha_hasta.
      - modo_reproceso='upsert_rango': actualiza/inserta el rango.
      - modo_reproceso='replace_rango': reemplaza completamente el rango en destino.
    """
    ...
    log = get_run_logger()
    log.info(
        "[INICIO] actualizar_base_ventas_extendida | %s | retry=%s | timeout_ms=%s",
        runtime["description"],
        RETRY_ATTEMPTS,
    ...
    t_insert_stg.with_options(name="Insertar STG").submit(
        window_days=window_days,
        fecha_desde=runtime["fecha_desde"],
        fecha_hasta=runtime["fecha_hasta"],
    ).result()
    ...
    if runtime["modo_reproceso"] == "replace_rango":
        t_replace_destino_rango.submit(
            fecha_desde=runtime["fecha_desde"],
            fecha_hasta=runtime["fecha_hasta"],
        ).result()
...
if __name__ == "__main__":
    # Uso:
    #   python actualizar_base_ventas_extendida.py
    #   python actualizar_base_ventas_extendida.py 30 true
    #   python actualizar_base_ventas_extendida.py 14 true 2026-02-01 2026-02-28 upsert_rango
    #   python actualizar_base_ventas_extendida.py 14 true 2026-02-01 2026-02-28 replace_rango
