...
def cargar_articulos_proveedores(lista_ids):
    ...
    query = f"""
        ...
        INNER JOIN [DIARCOP001].[DiarcoP].[dbo].[T050_ARTICULOS] A ON A.C_ARTICULO = S.C_ARTICULO
        LEFT JOIN [DIARCOP001].[DiarcoP].[dbo].[T060_STOCK] ST ON ST.C_ARTICULO = S.C_ARTICULO AND ST.C_SUCU_EMPR = S.C_SUCU_EMPR
        LEFT JOIN [DIARCOP001].[DiarcoP].[dbo].[T052_ARTICULOS_PROVEEDOR] AP ON A.C_PROVEEDOR_PRIMARIO = AP.C_PROVEEDOR AND S.C_ARTICULO = AP.C_ARTICULO
        LEFT JOIN [DIARCOP001].[DiarcoP].[dbo].[T055_ARTICULOS_CONDCOMPRA_COSTOS] CCP ON A.C_PROVEEDOR_PRIMARIO = CCP.C_PROVEEDOR AND S.C_ARTICULO = CCP.C_ARTICULO AND S.C_SUCU_EMPR = CCP.C_SUCU_EMPR
        LEFT JOIN [DIARCOP001].[DiarcoP].[dbo].[T710_ESTADIS_REPOSICION] R ON R.C_ARTICULO = S.C_ARTICULO AND R.C_SUCU_EMPR = S.C_SUCU_EMPR
