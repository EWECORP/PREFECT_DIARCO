...
BEGIN
    ...
    BEGIN
        ...
        SET @q_factor_vk        = TRY_CONVERT(DECIMAL(3,0), @q_factor);  -- <-- NUEVO

        BEGIN TRY
            IF @q_factor_proveedor IS NULL OR @q_factor_vk IS NULL
            BEGIN
                SET @RC = 99;
                SET @vchMensaje = 'q_factor fuera de rango para VK (DECIMAL(3,0)).';
            END
            ...
            BEGIN
                EXEC @RC = [10.54.200.88].[VALKIMIA].[dbo].[SD03_TRANSF_ALTA_DETALLE_VK]
                    ...
                    @q_bultos            = @q_bultos,
                    @q_factor            = @q_factor_vk,              -- <-- CORREGIDO
            ...
            BEGIN
                UPDATE [repl].[TRANSF_CONNEXA_IN]
                   SET estado_vk='PROCESADO', mensaje_error_vk='', f_procesado_vk=SYSDATETIME()
                 WHERE id=@id;
