...
SELECT name,CONVERT(nvarchar(4000),value) value,CONVERT(nvarchar(4000),value_in_use) value_in_use,description FROM sys.configurations;
GO

-- cdc_instancias
SELECT object_id,source_object_id,capture_instance,start_lsn,supports_net_changes,role_name,index_name,filegroup_name,create_date FROM cdc.change_tables;
GO

-- cdc_columnas
SELECT object_id,column_name,column_id,column_type,column_ordinal FROM cdc.captured_columns;
GO

-- logins
SELECT name,type_desc,is_disabled,default_database_name,default_language_name,CONVERT(varchar(200),sid,1) sid FROM sys.server_principals WHERE type IN ('S','U','G');
GO

-- roles_servidor
SELECT SUSER_NAME(role_principal_id) rol,SUSER_NAME(member_principal_id) miembro FROM sys.server_role_members;
GO