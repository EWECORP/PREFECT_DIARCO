ALTER TABLE IF EXISTS src.t020_proveedor
...
BEGIN
    ...
    WHERE table_schema = 'src'
      AND table_name = 't020_proveedor'
      AND column_name = 'cdc_lsn';

    IF v_cdc_lsn_type IS NOT NULL AND v_cdc_lsn_type <> 'bytea' THEN
        RAISE EXCEPTION
            'La columna src.t020_proveedor.cdc_lsn debe ser BYTEA para almacenar LSN de SQL Server. Tipo actual: %',
            v_cdc_lsn_type;
    END IF;
END $$;

COMMENT ON COLUMN src.t020_proveedor.fuente_origen IS
'Origen del proceso CDC que publico el registro.';

COMMENT ON COLUMN src.t020_proveedor.fecha_extraccion IS
'Fecha y hora de aplicacion del cambio en PostgreSQL.';

COMMENT ON COLUMN src.t020_proveedor.cdc_lsn IS
'LSN binario del cambio aplicado desde SQL Server CDC.';

COMMENT ON COLUMN src.t020_proveedor.estado_sincronizacion IS
'Estado tecnico del registro publicado por CDC. 0=aplicado, 1=pendiente, 2=error, 9=reseed_required.';

CREATE INDEX IF NOT EXISTS t020_proveedor_cdc_lsn_idx
    ON src.t020_proveedor (cdc_lsn);

CREATE UNIQUE INDEX IF NOT EXISTS t020_proveedor_pk_operativa_uidx
    ON src.t020_proveedor (c_proveedor);
